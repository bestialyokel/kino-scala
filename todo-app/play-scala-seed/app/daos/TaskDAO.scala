...
class TaskDAODatabaseImpl @Inject() (val dbConfigProvider: DatabaseConfigProvider)(implicit
  ...
  def getAll(): Future[Seq[Task]] = db.run(Tasks.filter(_.deleted.isEmpty).result)

  def getOneById(id: Int): Future[Option[Task]] = db.run(Tasks.filter(_.id === id).result.headOption)

  def create(dto: CreateTaskDTO): Future[Option[Task]] = db
    .run((Tasks.returning(Tasks) += dto.transform).map(Some(_)))

  def updateNameById(id: Int, dto: UpdateTaskNameDTO): Future[Unit] = db
    .run(Tasks.filter(_.id === id).map(_.name).update(dto.name))
    .void

  def updateStatusById(id: Int, dto: UpdateTaskStatusDTO): Future[Unit] = db
    .run(Tasks.filter(_.id === id).map(_.status).update(dto.status))
    .void

  def setStatusForAll(status: TaskStatus): Future[Unit] = db
    .run(Tasks.map(_.status).update(status))
    .void

  def deleteById(id: Int): Future[Unit] = db
    .run(Tasks.filter(_.id === id).map(_.deleted).update(OffsetDateTime.now().some))
    .void
...
trait TaskComponent {
  ...
  protected class TasksTable(tag: Tag) extends Table[Task](tag, "tasks") {
